...
export const DeactivateCategory = ({ id }: Props) => {
  const handleDeactivate = async () => {
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.error || "Error al desactivar la categoría");
      }
    } catch (error) {
      console.error(error);
    }
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Esta acción no se puede deshacer. Esto marcará la categoría como
