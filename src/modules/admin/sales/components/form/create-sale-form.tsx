...
import { useSaleCartStore } from "../../store/sale-cart.store";
...
export const CreateSaleForm = ({
  ...
  const cartItems = useSaleCartStore((s) => s.items);
  const addArtwork = useSaleCartStore((s) => s.addArtwork);
  const removeArtwork = useSaleCartStore((s) => s.removeArtwork);
  const clearCart = useSaleCartStore((s) => s.clear);
  ...
  const handleSubmit = async (formData: CreateSaleDto) => {
    ...
    try {
      ...
      const safeFormData: CreateSaleDto = {
        ...formData,
        subtotal,
        tax,
        total,
      ...
      const result = await createSaleAction(safeFormData, safeCartItems);
